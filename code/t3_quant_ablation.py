"""Controlled experiment: vary quantization resolution n -> does the T3 gain shrink like a dead-zone (e*=L*tan(pi/n))?
(A) if dead-zone dither, effect ~ dead-zone -> drops sharply as n grows. (B) if rate-damping, independent of n."""
...
print("dead-zone width e*=L*tan(pi/n):", {n:round(AL.LOOK*np.tan(np.pi/n),3) for n in [8,16,32]})
print("\n(A) dead-zone: effect drops as n grows / (B) rate-damping: independent of n. delay26 tr0.3")
for tn,t in [('circle',Circle()),('square',Square())]:
    print(f"\n== {tn} ==")
    print(f"{'n':>3} {'e*':>6} | {'honest':>7} {'T3':>7} | {'gain%':>7}")
    for n in [8,16,32]:
