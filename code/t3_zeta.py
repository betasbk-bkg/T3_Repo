"""(2) effective damping ratio ζ 추출: signed 오차 자기상관의 로그-감쇠법.
underdamped 2차계: 자기상관 peak들이 exp(-ζω τ)로 감쇠 -> log-decrement δ -> ζ=δ/sqrt((2π)^2+δ^2)."""
...
def zeta_from_series(e):
    ...
    # 연속 peak 비율로 log-decrement (여러개 평균)
    deltas=[]
...
print("=== effective damping ratio ζ (circle N50 tr0.3 d26, log-decrement) ===")
zh,sh,nh=zeta_avg('honest'); zt,st,nt=zeta_avg('T3')
print(f"  honest: ζ = {zh:.3f} ± {sh:.3f}  (n={nh})")
print(f"  T3    : ζ = {zt:.3f} ± {st:.3f}  (n={nt})")
print(f"  => Δζ = {zt-zh:+.3f}  ({(zt-zh)/zh*100:+.0f}%)  {'T3가 감쇠비↑ = damping 확정' if zt>zh else '미확정'}")
