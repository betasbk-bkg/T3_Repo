...
This manifest maps archive files to manuscript claims and reproducibility tasks.
...
- `latency_summary.csv`: latency-level summary.
...
Earlier draft figure scripts that used superseded terminology such as physical velocity damping are not used for the final figure set. The final manuscript interpretation is projected local error relief after vector normalization, not physical speed damping.
